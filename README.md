...
「AからBまでT_max分で移動した」という実績をもとに、移動途中で立ち寄れた可能性のあるエリアを 125m メッシュ（L6）で出力するツール。
...
### サンプル実行（埼玉県庁 → 東松山市役所・T_max=60,65,70分）
...
`t_arrive_earliest` 〜 `t_arrive_latest` がそのメッシュに存在可能な時刻帯となる。`slack_min` が大きいメッシュほど滞在・立ち寄りの余裕があった場所として優先度を高く評価できる。

**通過可能メッシュ（赤色）の算出条件**

```
dist_a_min + dist_b_min ≤ T_max  （= slack_min ≥ 0）

例: 28.3分 + 25.1分 = 53.4分 ≤ 60分 → 通過可能（赤色）
例: 40.0分 + 25.0分 = 65.0分 > 60分 → 通過不可（非表示）
```

「始点からそのメッシュを経由して終点まで T_max 分以内に到達できるか」の判定。T_max を大きくするほど赤色エリアが広がる。
